...
Junto vai [`core/services.py`](../../../core/services.py), que monta `os.getenv('USERS_MS') + '/api/'` **no nível do módulo** — é por isso que a aplicação exige `USERS_MS` para subir, mesmo apontando para um host que não resolve ([11](../../concepts/11_open_issues_and_technical_debt.md) §9).
...
- [ ] **Docs atualizados:** docs [01](../../concepts/01_system_overview.md), [02](../../concepts/02_backend_architecture.md) e [08](../../concepts/08_heroku_backend.md) — a seção "as duas variáveis que derrubam o boot" passa a ter uma só; doc [11](../../concepts/11_open_issues_and_technical_debt.md) §5 e §9 para "Já resolvido".
